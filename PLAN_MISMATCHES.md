...
- 省心档（LITE）：仅 WorkManager，组件有 60 秒内的新码就显示（`hasFreshCode`），由用户点刷新按钮或打开 App 时获取。
- 稳妥档（STEADY）：WorkManager + 前台服务（30 秒密集刷新）。

**理由**:付款码只有约 1 分钟有效期，WorkManager 无法维持组件付款码的新鲜度。省心档由用户按需获取，有新码即显、到期即隐；稳妥档持续刷新动态码。

**默认档位**:省心档。需要组件持续展示动态码时由用户切换到稳妥档。

**组件过期**:精确闹钟是纯撤码优化——展示新码时安排在 `cachedAt + 60 秒` 触发组件重绘隐藏过期码，不执行联网请求。拿不到精确闹钟权限时静默跳过，组件仍按 `hasFreshCode` 时效显码，任何一次重渲染都会重判过期，闹钟只是让隐藏更及时。稳妥档服务正常运行时，新码会持续替换上一条闹钟。

**精确闹钟权限声明**:只保留 `SCHEDULE_EXACT_ALARM`（`maxSdkVersion=32`），不声明 `USE_EXACT_ALARM`。后者是 Google Play 敏感权限，仅限闹钟/日历类应用，撤码优化这种用途会被拒审。代价是 Android 13+ 上 `canScheduleExactAlarms()` 默认返回 false，精确撤码在 13+ 默认不生效——但这只影响过期码隐藏的及时性，可扫性由服务端校验兜底，不算功能缺失。
...
**决策**:`QrGenerator` 定义三档语义化尺寸常量（`SIZE_WIDGET=288` / `SIZE_CARD=600` / `SIZE_FULLSCREEN=700`），替代散落的魔法数字。二维码是矢量矩阵，尺寸只影响位图清晰度，不影响扫码内容。组件档取 288 是为压低 RemoteViews 经 Binder 传输的事务体积，降低撞 1MB 上限导致组件无声不刷新的概率。
