...
**现状**:图标集里补了 `ic_dots`（更多菜单）、`ic_edit`（编辑）、`ic_search`（搜索）、`ic_file`（文件），但账号列表当前交互仍是「点击切换、长按删除」，未加入每项的更多菜单入口。

**决策**:本轮只统一图标资源与视觉语言，不新增交互面。这几枚图标作为后续「列表项操作菜单/搜索/导入文件」等功能的现成资源，避免风格再次分裂。

**待验证**:是否需要把长按删除升级为更多菜单（编辑/删除/置顶），取决于多卡场景的实际操作频率。
